...
**진행**: A 도메인 규칙 · B CoreData/저장소 · C 온보딩 · D 캐릭터 화면 완료(단위 테스트 76개) · 남은 것은 E-2 수동 검증

> **2026-08-18 정책 개정** — 화면으로 밸런스를 보고 네 가지를 바꿨다. 아래 A-2·A-4·A-5·B-2·B-3·D-2 항목 중 개정 전 규칙을 적은 줄에는 `→ 개정` 표시를 달았다.
>
> 1. **레벨 요구량이 고정 표로 바뀌고 Lv.7이 최고 레벨이다.** `1,000 / 2,105 / 3,347 / 5,063 / 7,423 / 10,639 / 36,189`. 등차수열(`100 * (level+1)`)과 `levelStartExperience`는 삭제했다.
> 2. **레벨업 시 경험치는 `0`이 되고 이월되지 않는다.** 그래서 `Pet.totalExperience`(누적)가 `Pet.experience`(현재 레벨 적립분)로 바뀌었다. CoreData 속성명은 마이그레이션 없이 `totalExperience`로 남고 `PetMapper`가 흡수한다.
> 3. **경험치 수치는 UI에 노출하지 않는다.** 캐릭터 화면의 `현재 / 필요 EXP` 라벨과 학습 완료 카드의 `보유 포인트` 항목을 없앴다(`ExperienceGain.totalPoint`까지 정리). 진행 정도는 게이지로만 알린다.
> 4. **돌보기는 대상 수치가 `80` 미만일 때만 가능하다**(`PetStatePolicy.careThreshold`). 100 유지가 최적 전략이 되는 걸 막는다. 그 대신 **생성 시 돌봄 수치를 `100` → `70`**(`initialStat`)으로 낮춰 온보딩 직후 바로 돌볼 수 있게 했다 — `80` 미만이라 버튼이 열리고 `healthyThreshold(65)` 초과라 기분이 "만족함"이며 HP 회복 조건도 만족한다.

## 현재 상태

- ~~캐릭터 화면은 테스트 레이블과 닫기 버튼만 있고 ViewModel이 없다.~~ → D에서 `CharacterViewModel` + 상태 UI + 액션 6종을 붙였다. `MainCoordinator.didTapCharacter()`는 여전히 진입마다 새 인스턴스를 full-screen present 한다.
...
- 알·펫 이미지 에셋은 아직 없다. C-1 알 선택 셀은 **이미지를 그리지 않고** 테두리와 가운데 텍스트만 표시하므로 `PetType.eggImageName`은 현재 참조하는 곳이 없다. `imageName`은 D-2 캐릭터 화면에서 쓰지만 에셋이 없어 `UIImage(named:) ?? UIImage(systemName: "pawprint")`로 대체 중이다. 하트도 같은 이유로 SF Symbol `heart.fill`을 부분 클리핑해 그린다.
...
- 초기 돌봄 수치는 ~~`100`~~ **`70`**, HP는 `40`, 시작 레벨과 경험치는 `0`이다. → 개정 4
...
- [x] 레벨, 누적 경험치, HP는 저장하고 기분·게이지 진행률·사망 여부·하트 표시 상태는 파생값으로 둔다.
  - 하트는 `PetHeartPolicy.hearts(hp:)`가 `[PetHeartFill]` 10칸을 돌려주고 `PetDisplayInfo.hearts`가 실어 나른다. 저장 컬럼은 늘지 않았다.
- [x] 화면 표시 모델 `PetDisplayInfo`는 `Shared/Domain/Entities/`에 둔다. Shared의 UseCase가 반환하므로 `Feature/Character`에 두면 의존 방향이 역전된다 (`VocabDisplayInfo` 패턴).
  - `hearts` 추가로 필드가 확정됐다. `init(pet:)`가 파생값 6종을 한곳에서 조립하므로 UseCase 4개는 손대지 않았다.
...
- [x] 대상 수치가 정산 후 이미 ~~`100`~~ **`80` 이상**이면 다른 수치와 HP 정산 결과만 저장하고 "이미 충분히 좋아요" 결과를 반환한다. → 개정 4
...
- [x] 화면 조회·포그라운드 복귀·돌보기·레벨업 시에만 한 번 계산하고 저장한다. 백그라운드 작업과 상시 타이머는 사용하지 않는다.
  - `CharacterViewModel`이 `viewWillAppear`와 `didBecomeActiveNotification` 두 시점만 `merge`해 `FetchPetStateUseCase`를 부른다. 타이머는 없다.
...
- [x] ~~`PetLevelPolicy`에 레벨별 요구량을 `requiredExperience(level) = 100 * (level + 1)`로 정의한다.~~ → **개정 1: 고정 표 + `maxLevel = 7`**
- [x] ~~현재 레벨 시작 전 누적 요구량을 `levelStartExperience(level)`로 계산한다.~~ → **개정 2로 삭제** (경험치가 레벨마다 0에서 시작하므로 누적 기준점이 없다)
- [x] 게이지는 ~~`(totalExperience - levelStartExperience) / requiredExperience`~~ **`experience / requiredExperience`**를 `0...1`로 제한해 계산한다. 최고 레벨은 항상 `1`이다. → 개정 1·2
- [x] 누적 경험치가 다음 경계 이상일 때만 레벨업 버튼을 활성화하고, 탭 시 조건을 다시 검사한 뒤 `level += 1`만 저장한다.
  - 버튼은 `isEnabled = info.canLevelUp && !pet.isDead`. 재검사는 `LevelUpPetUseCase`가 하고 미달이면 `notEnoughExperience` 토스트를 띄운다.
- [x] ~~레벨업 시 누적 경험치는 차감하지 않는다. 초과 경험치는 다음 레벨 게이지로 이월한다.~~ → **개정 2: 승급하면 `experience = 0`이고 초과분은 버린다.** 여러 레벨 분량을 모아도 한 번만 오른다. 최고 레벨에서는 `canLevelUp`이 항상 `false`라 버튼을 숨긴다. 부활 페널티는 A-5 그대로.
...
- [x] 표시용 칸 수는 `hp <= 0`이면 `0`, 그 외에는 `max(1, floor(hp))`로 계산한다. 완전한 하트 수는 `floor(표시용 칸 / 4)`, 남은 칸은 `표시용 칸 % 4`로 파생한다.
  - `PetHeartPolicy`(`Shared/Domain/UseCases/`). 순수 계산이라 테스트 타깃에 RxSwift 없이 검증된다.
- [x] 남은 칸 `3 / 2 / 1`은 각각 `2/3 / 1/2 / 1/3` 하트로 표시한다. `0 < hp < 1`에서도 최소 1칸인 `1/3` 하트 하나를 보장하고 `hp == 0`에서만 모두 소멸한다.
...
- [x] 현재 레벨의 경험치가 `0%`보다 크면 부활 시 해당 레벨 요구 경험치의 `10%`를 차감하고 ~~현재 레벨 시작 경험치~~ **`0`** 아래로 내려가지 않게 한다. → 개정 2 (하한만 바뀜)
- [x] 현재 레벨의 경험치가 이미 `0%`이면 부활 시 레벨을 한 단계 내리고 낮아진 레벨의 `0%`로 맞춘다(내려간 레벨의 적립분은 복원하지 않는다 — 이월이 없는 것과 같은 이유다). 레벨 0에서는 더 낮추지 않는다.
...
- [x] Repository는 CRUD만 맡는다: 펫 생성, 조회, 전체 저장, 경험치 가산. **정산·조건 판단·페널티 계산은 넣지 않는다** — 정책 판단은 B-3의 UseCase 책임이다.
  - `createPet`은 완성된 `Pet`을 받는다. 초기 수치 ~~`100`~~ **`70`(`initialStat`)**·HP `40`은 `PetStatePolicy` 상수라서 `CreatePetUseCase`가 조립해 넘긴다 — Repository는 밸런스값을 모른다.
...
- [x] `Shared/Domain/UseCases/`에 `protocol X` + `final class DefaultX: X` 쌍으로 추가하고 `AppDIContainer`에 `make*()`를 넣는다 (기존 10개와 동일 형태).
  ...
  - `CarePetUseCase` — `PetCareStat` 하나를 받아 정산 후 `+25`. 이미 ~~`100`~~ **`80` 이상**이면 정산분만 저장하고 안내 결과 반환
...
- [x] 사망 · `100` 도달 · 조건 미충족 같은 액션 결과는 UseCase가 결과 타입으로 돌려주고 ViewModel은 표시만 한다.
  ...
  - `PetDisplayInfo`는 `VocabDisplayInfo`처럼 도메인 엔티티(`pet`)를 품고 파생값(`mood`·게이지·`canLevelUp`)만 더한다. 하트 표시 상태(`hearts`)도 D-2에서 여기 합류했다.
...
> 단위 테스트로는 확인 완료. 화면(D-1)이 붙어 **수동 검증이 가능해졌다** → E-2.
...
- [x] `CharacterViewModel`을 Input/Output 방식으로 추가하고 B-3의 UseCase들을 주입한다. `PetRepository`를 직접 참조하지 않는다.
  - 의존성은 조회·돌보기·레벨업·부활 UseCase 4개뿐. `CreatePetUseCase`는 온보딩 몫이라 받지 않는다.
- [x] Input은 화면 표시·앱 활성화·돌보기 종류·레벨업 탭·부활 탭으로 제한한다. Output은 이름·이미지·레벨·경험치 진행률·네 돌봄 수치·기분·HP·하트 표시 상태·사망 여부·버튼 상태·안내 메시지를 제공한다.
  - **Output은 `Driver<PetDisplayInfo>` 하나 + `Signal<String>`(토스트) 둘뿐이다.** 나열된 11가지를 `PetDisplayInfo`가 이미 전부 담고 있어 Driver를 11개로 쪼개면 같은 값을 11번 `map`하는 보일러플레이트가 되고, 액션 하나의 결과가 11개 스트림으로 흩어져 렌더링 원자성도 잃는다. VC의 `render(_:)`가 한 번에 분배한다.
  - 돌보기 4버튼은 `Observable.merge`로 묶어 `Observable<PetCareStat>` 하나로 보낸다 (`PetCareStat`이 `CaseIterable`이라 버튼·행·색이 전부 같은 배열에서 나온다).
- [x] `viewWillAppear`와 `UIApplication.didBecomeActiveNotification`에서 상태를 다시 조회한다. 화면을 계속 켜 둔 동안의 분 단위 타이머는 넣지 않는다.
  - `rx.methodInvoked(#selector(viewWillAppear))` — 기존 4개 화면과 같은 관용구.
- [x] `MainCoordinator`가 `AppDIContainer.makeCharacterViewModel()`로 ViewModel을 생성해 기존 full-screen 화면에 주입한다. 지금은 진입마다 `CharacterViewController`가 새로 생성되므로 Relay 없이도 퀴즈 경험치가 재조회로 반영된다 — 나중에 진짜 `UITabBarController`로 바꾸면 뷰컨이 살아남아 이 전제가 깨진다.
- [x] 캐릭터 화면이 하나뿐이므로 별도 `CharacterCoordinator`는 만들지 않는다.

### D-2. 상태 UI

- [x] 캐릭터 이미지, 펫 이름, 현재 기분, 레벨을 표시한다.
  - 요소가 많아 좁은 기기에서 넘치므로 `UIScrollView` + 세로 `UIStackView` 하나로 세웠다.
- [x] ~~`UIProgressView`로~~ **`CustomProgressView`로** 현재 레벨 경험치 게이지를 표시한다. ~~`현재 EXP / 필요 EXP` 텍스트도 함께 표시한다.~~ → **개정 3으로 라벨 삭제, 게이지만 남았다.**
  - 클램프는 이미 `PetLevelPolicy.progress`(`min(1,…)`)와 `setProgress`가 이중으로 한다. 화면에 다시 쓰지 않았다.
  - `PetDisplayInfo`에서 `currentExperience`·`requiredExperience`를 지우고 `isMaxLevel`을 넣었다 — 숫자를 안 쓰니 소비처가 사라졌고, 최고 레벨에서 레벨업 버튼을 숨겨야 한다.
  - **절대 수치 대신 진행률만 퍼센트로 표시한다** (`Lv. 3` 오른쪽에 `62%`). 게이지만으로는 진행 정도를 눈대중해야 했다.
    - 퍼센트는 **버림**이다. 반올림하면 `99.6%`가 `100%`로 떠서 레벨업 버튼은 비활성인데 다 찬 것처럼 읽힌다 — `progress == 1`일 때만 `100%`가 되도록 맞췄다. 같은 화면의 `CareStatRowView`가 반올림을 쓰는 것과 반대이고, 그 이유는 코드 주석에 남겼다.
- [x] 포만감·수분·즐거움·청결 수치와 게이지를 표시한다.
  - `CareStatRowView(stat:)` 한 컴포넌트를 네 번 쓴다 — `[포만감] [====게이지====] [82]`.
  - **수치 텍스트는 버림이 아니라 반올림한다.** 버림으로 두면 갓 만든 펫의 `99.96`이 `99`로 떠서 게이지는 꽉 찼는데 숫자만 모자라 보이고, 돌보기가 `+25`가 아니라 `+1`만 하는 것처럼 읽힌다(실제로는 `99.96 → 100` 클램프). 육안 검증 중 실제로 이 오해가 나왔다.
  - **`PetCareStat.hydration`의 `title`을 "갈증" → "수분"으로 고쳤다.** 네 수치는 전부 100이 좋은 긍정 방향인데 "갈증 100"만 반대로 읽힌다. 화면이 `title`의 첫 사용처라 참조 회귀는 없다.
- [x] 돌봄 수치가 `20 이하`면 위험 표시와 함께 HP가 감소 중임을 텍스트로 알린다. 기분만으로는 사망이 임박한 것을 알 수 없고 푸시 알림도 이번 범위 밖이다.
  - 임계값은 `PetStatePolicy.dangerThreshold`를 읽는다. 화면에 `20`을 다시 쓰지 않았다.
- [x] HP를 최대 10개의 하트로 표시한다. 표시용 칸 `4 / 3 / 2 / 1`을 각각 `가득 참 / 2/3 / 1/2 / 1/3` 이미지로 표현하고, 남은 칸이 `0`이면 부분 하트를 제거한다.
  - 계산은 `PetHeartPolicy`, 표시는 `HeartBarView`. 테스트 7개도 함께 만들었다.
  - **하트 에셋이 없어 SF Symbol `heart.fill`을 두 겹으로 깔고 위쪽을 폭으로 잘라 부분 채움을 만든다.** `heart.righthalf.filled`까지 써도 3단계뿐이라 `2/3`·`1/3`을 표현할 수 없다.
    - 채움 이미지는 클리핑 뷰가 아니라 **슬롯 크기에 못박아야** 한다. 클리핑 뷰에 맞추면 잘릴 때 하트가 가로로 찌그러진다.
    - 에셋이 들어오면 바꿀 곳은 `HeartBarView.makeSlot()` 하나다.
- [x] `0 < hp < 1`이면 `1/3` 하트 하나를 표시하고 `hp == 0`이면 하트를 모두 소멸시킨다. 빈 하트는 HP 단계가 아니라 빈 배경 슬롯 또는 소멸 전환 표현으로만 사용한다.
  - `hearts(hp:)`가 항상 10칸을 돌려주고 뒤쪽 `.empty`가 곧 배경 슬롯이다.
- [x] ~~Quiz Feature 전용 `CustomProgressView`는 직접 참조하지 않는다.~~ → **`Shared/DesignSystem/Components/`로 옮겼다.**
  - 내용이 퀴즈와 무관한 범용 게이지고 이제 5개 게이지가 쓴다. `QuizViewController` 하단의 `Reactive` 바인더 extension도 컴포넌트 파일로 함께 옮겼다.
  - fill 색이 `AppColor.black` 하드코딩이라 `init(fillColor: UIColor = AppColor.black)`로 열었다. 기본값이라 퀴즈 동작은 그대로다.
  - 돌봄 4색은 `PetCareStat+Color.swift`(`BookTopic+Color` 패턴). Palette에 남아 있던 `butter/sky/coral/sage`를 써서 새 컬러셋은 추가하지 않았다.
- [x] 게이지와 비활성·개발 중 상태가 색상만으로 전달되지 않도록 텍스트와 VoiceOver 값을 함께 제공한다.
  - 돌봄 행은 `accessibilityValue = "82퍼센트"`(위험 시 `", 위험"` 덧붙임), 경험치 섹션은 `"레벨 3, 62퍼센트"`(최고 레벨은 `"레벨 7, 최고 레벨"`). 숫자를 화면에서 뺐어도 게이지만 남으면 VoiceOver가 읽을 게 없어지므로 퍼센트로 대체했다. 위험은 색과 문구 양쪽으로 알린다.
- [x] 사망 시 캐릭터 사망 상태와 부활 버튼을 표시하고 VoiceOver에 현재 HP와 사망 여부를 전달한다.
  - 기분 라벨을 "세상을 떠났어요"로 갈아치우므로 VoiceOver가 그대로 읽는다. 하트 그룹은 `accessibilityValue = "40 중 37"`.

### D-3. 액션

- [x] 밥주기 → 포만감, 물주기 → 수분, 놀아주기 → 즐거움, 청소하기 → 청결을 회복한다.
  - 버튼 문구는 `PetCareStat.actionTitle`. 수치 이름(`title`)과 행동 이름을 분리했다.
  - **버튼은 2×2 그리드다.** 한 줄에 넷을 놓으면 좁은 기기에서 한 칸이 70pt라 "놀아주기"가 잘린다.
- [x] 돌보기 결과를 즉시 다시 렌더링하고 이미 ~~100~~ **`80` 이상**이면 토스트로 안내한다. → 개정 4
  - **화면이 자체 임계값을 갖지 않는다.** `careThreshold` 하나를 버튼 활성 조건(`값 < 80`)과 `care()`의 거절 조건(`값 >= 80`) 양쪽이 함께 읽는다. 버튼이 이미 잠겨 있으므로 `alreadyFull` 토스트는 레이스 방어용으로만 남는다.
  - 토스트는 `BaseViewController.showToast(_:)` 직접 호출이다. `ToastManager`를 경유하지 않는다 — 아래 발견 사항 참조.
- [x] 경험치 게이지가 100% 미만이면 레벨업 버튼을 비활성화한다. **최고 레벨(Lv.7)에서는 아예 숨긴다.** → 개정 1
  - `PrimaryFillButton`의 `configurationUpdateHandler`가 비활성 색을 이미 처리하므로 `isEnabled` 한 줄이다. 숨김은 `isHidden = info.isMaxLevel`.
- [x] 레벨업 성공 후 레벨·게이지·버튼 상태를 다시 렌더링하고 ~~초과 경험치를 보존한다~~ **게이지를 0으로 되돌린다**. → 개정 2
- [x] 사망 상태에서는 돌보기·레벨업 버튼을 비활성화하고 부활 버튼만 활성화한다. 학습 화면 진입과 퀴즈 경험치 적립은 막지 않는다.
  - 부활 버튼은 평소 `isHidden`이고 사망 시에만 드러난다.
- [x] 부활 성공 후 HP 40, 하트 10개, 돌봄 수치 `50`, 경험치 차감 또는 레벨 하락 결과를 즉시 다시 렌더링한다.
  - **되돌릴 수 없는 차감이라 `AlertPresenter.showAlert` 확인을 거친다.** 확인한 탭만 `reviveConfirmed` Relay로 ViewModel에 들어간다.

> **검증 기준**: 앱을 닫아 둔 시간만큼 상태와 HP가 한 번만 정산되고, 경험치가 부족할 때는 저장소를 직접 호출해도 레벨이 오르지 않으며, 살아 있는 펫은 부활할 수 없어야 한다.
> 단위 테스트와 빌드는 통과. **육안 검증은 E-2 남음.**

> **작업 중 발견 (D 범위 밖이라 고치지 않음)**: `BaseViewController.setupToastObserver()`가 `private`으로 선언만 되고 **아무 데서도 호출되지 않는다.** 즉 `ToastManager.shared.show(_:)`로 보낸 토스트는 지금 어느 화면에서도 뜨지 않는다. 캐릭터 화면은 `showToast(_:)` 직접 호출로 우회했다. 고치려면 다른 화면의 기존 `ToastManager` 호출부가 갑자기 토스트를 띄우기 시작하므로 회귀 확인이 필요하다.
...
> **현재 76개 통과** — `PetStatePolicyTests`(34) · `PetPersistenceTests`(21) · `PetLevelPolicyTests`(7) · `PetNamePolicyTests`(7) · `PetHeartPolicyTests`(7).
...
- [x] `PetLevelPolicy`: 요구량 표 7개, 레벨 0의 999/1,000 EXP, 레벨 1의 2,105 EXP, 최고 레벨(승급 불가·게이지 1), 승급 후 경험치 0이라 연속 승급이 불가한 것을 테스트한다. → 개정 1·2
...
- [x] HP 표시 파생값을 `40 / 39 / 38 / 37 / 36 / 0.5 / 0`에서 검증해 하트 10개, `2/3 · 1/2 · 1/3`, 최소 1칸, 전멸 상태를 확인한다.
  - `PetHeartPolicyTests`(7). 배열 길이가 항상 10인 것과 소수점 버림(`37.9 == 37`)도 함께 본다.
- [x] 부활 시 요구량의 10% 차감(레벨 1 → `-210`), 차감량보다 적게 모았으면 `0`, 진행률 0%에서 한 단계 하락, 레벨 0 하한, 승급을 미뤄 초과분이 쌓인 경우, 살아 있는 펫의 부활·중복 요청 거부를 테스트한다.
...
- [ ] 퀴즈 완료 경험치가 캐릭터 게이지와 레벨업 버튼에 반영되는지 확인한다. **경험치 숫자가 어디에도 보이지 않아야 한다** (캐릭터 화면 라벨, 학습 완료 카드 `보유 포인트` 모두 제거).
- [ ] 온보딩 직후 네 돌봄 수치가 `70`, 기분이 "만족함", 돌보기 4버튼이 모두 활성인지 확인한다. → 개정 4
- [ ] 돌보기를 누르면 `70 → 95`가 되고 그 버튼이 즉시 비활성으로 바뀌는지 확인한다(95 ≥ 80).
- [ ] 학습 완료 카드에서 `보유 포인트`가 빠진 뒤 `획득 경험치` 한 항목이 가운데 정렬로 남고 레이아웃이 깨지지 않는지 확인한다.
- [ ] 경험치를 1,000 이상 모아 레벨업하면 Lv.1이 되고 **게이지가 0으로 초기화**되는지(초과분 미이월) 확인한다. → 개정 2
- [ ] DB에 `ZLEVEL = 7`을 심고 진입해 게이지가 꽉 차고 레벨업 버튼이 보이지 않는지 확인한다. → 개정 1
...
3. ~~레벨 곡선: 별도 표가 없으면 레벨마다 요구량이 100씩 증가하는 A-4 공식을 사용한다.~~ → **표를 받아 개정 1로 확정.**
